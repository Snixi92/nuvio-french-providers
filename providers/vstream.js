...
function detectEndpoint() {
  ...
  return fetch(DOMAINS_URL)
    ...
    .then(function(d) {
      ...
      _cachedEndpoint = {
        base: 'https://vstream.' + tld,
        api:  'https://api.vstream.' + tld + '/api',
        ref:  'https://vstream.' + tld + '/'
      };
      return _cachedEndpoint;
    })
    .catch(function() {
      _cachedEndpoint = {
        base: 'https://vstream.' + FALLBACK_TLD,
        api:  'https://api.vstream.' + FALLBACK_TLD + '/api',
        ref:  'https://vstream.' + FALLBACK_TLD + '/'
      };
      return _cachedEndpoint;
    });
}
...
function getTmdbMeta(tmdbId, type) {
  return fetch('https://api.themoviedb.org/3/' + (type === 'tv' ? 'tv' : 'movie') + '/' + tmdbId + '?api_key=' + TMDB_KEY + '&language=en-US')
    ...
    .then(function(d) {
      return { name: d.title || d.name || 'Vstream', year: (d.release_date || d.first_air_date || '').split('-')[0] };
    }).catch(function() { return { name: 'Vstream', year: '' }; });
}
