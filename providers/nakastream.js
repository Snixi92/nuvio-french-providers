...
function detectEndpoint() {
  ...
  return fetch(DOMAINS_URL)
    ...
    .then(function(d) {
      ...
      _cachedEndpoint = {
        base: 'https://nakastream.' + tld,
        api:  'https://api.nakastream.' + tld + '/api',
        ref:  'https://nakastream.' + tld + '/'
      };
      return _cachedEndpoint;
    })
    .catch(function() {
      _cachedEndpoint = {
        base: 'https://nakastream.' + FALLBACK_TLD,
        api:  'https://api.nakastream.' + FALLBACK_TLD + '/api',
        ref:  'https://nakastream.' + FALLBACK_TLD + '/'
      };
      return _cachedEndpoint;
    });
}
...
function getTmdbMeta(tmdbId, type) {
  ...
  return fetch(url).then(function(r) { return r.json(); }).then(function(d) {
    return { name: d.title || d.name || 'Nakastream', year: (d.release_date || d.first_air_date || '').split('-')[0] };
  }).catch(function() { return { name: 'Nakastream', year: '' }; });
}
...
function getEpInfo(tmdbId, season, episode) {
  ...
  return fetch('https://api.themoviedb.org/3/tv/' + tmdbId + '/season/' + season + '/episode/' + episode + '?api_key=' + TMDB_KEY)
    ...
    .then(function(d) { return { name: d.name || null }; })
    .catch(function() { return null; });
}

function buildTitle(meta, lang, season, episode, epInfo) {
...
function normalizeSources(sources, endpoint, meta, season, episode, epInfo) {
  ...
  for (var i = 0; i < sources.length; i++) {
    ...
    out.push({
      ...
      title: buildTitle(meta, s.lang || 'VF', season, episode, epInfo),
      url: url,
      quality: quality,
